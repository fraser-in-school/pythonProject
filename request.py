import requests
import hashlib
import sys

secret_key = 'JniWQjitRfQZntCH'
domain_name = 'https://api.e.mi.com'
# 请求的 URL
update_url = '/openapi/tools/userPackage/update/tag'  # 请替换为实际的上传 URL

update_list_url = '/openapi/tools/userPackage/updateInfo/availableUpdateList'

state_url = '/openapi/tools/userPackage/updateInfo/get'


def build_param(customer_id, tag_id):
    # 请求参数
    params = {
        'customerId': customer_id,  # 第一个参数
        # 'model': 'overwrite',
        'signId': 'b741ef165103518ef3fe23a3aeaea071',
        'tagId': tag_id,   # 第二个参数
    }
    params['sign'] = get_str(params)
    # print(available_params['sign'])
    # 使用字典构建 formData
    return params


def get_str(param_map):
    base_str = ''
    keys = param_map.keys()
    keys = sorted(keys)
    for key in keys:
        base_str += key + str(param_map[key])
    base_str += secret_key
    print(base_str)
    md5_hash = hashlib.md5()
    md5_hash.update(base_str.encode('utf-8'))
    return md5_hash.hexdigest().lower()


def get_md5(base_str):
    md5_hash = hashlib.md5()
    md5_hash.update(base_str.encode('utf-8'))
    return md5_hash.hexdigest().lower()


def post_req(params, file_path):
    form_data = {
        'targetFile': (file_path, open(file_path, 'rb'))  # 将文件添加到 formData
    }
    response = requests.post(domain_name + update_url, files=form_data, params=params)
    response.encoding = 'utf-8'
    # 打印响应
    print(response.status_code)
    print(response.text)


def get_req(customer_id, tag_id):
    available_params = build_param(customer_id, tag_id)
    response = requests.get(url=domain_name + state_url, params=available_params)
    response.encoding = 'utf-8'
    # 打印响应
    print(response.status_code)
    print(response.text)


customer_id = 482192
tag_id = 285600
get_req(customer_id, tag_id)
param = build_param(customer_id, tag_id)
